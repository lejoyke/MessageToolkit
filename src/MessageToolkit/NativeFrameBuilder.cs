...
/// </summary>
/// <remarks>
/// 适用于数据类型已匹配的场景（如 IO 点位 bool[]），
/// 核心功能是地址映射和帧构建，不进行类型转换。
/// </remarks>
/// <typeparam name="TProtocol">协议结构体类型</typeparam>
/// <typeparam name="TData">原生数据类型</typeparam>
public sealed class NativeFrameBuilder<TProtocol, TData> : IFrameBuilder<TProtocol, TData>
    where TProtocol : struct
{
    ...
    /// 编解码器
    /// </summary>
    public IProtocolCodec<TProtocol, TData> Codec { get; }
    ...
    public NativeFrameBuilder(IProtocolSchema<TProtocol> schema, IProtocolCodec<TProtocol, TData> codec)
    {
    ...
    /// 构建写入整个协议的帧
    /// </summary>
    public IFrame<TData> BuildWriteFrame(TProtocol protocol)
    {
        return new NativeWriteFrame<TData>(Schema.StartAddress, Codec.Encode(protocol));
    }
    ...
    public NativeWriteFrame<TData> BuildWriteFrame(Expression<Func<TProtocol, TData>> fieldSelector, TData value)
    {
        var address = Schema.GetAddress(fieldSelector);
        return new NativeWriteFrame<TData>(address, value);
    }
    ...
    /// <param name="value">要写入的值</param>
    public NativeWriteFrame<TData> BuildWriteFrame(ushort address, TData value)
    {
        return new NativeWriteFrame<TData>(address, value);
    }
    ...
    public NativeWriteFrame<TData> BuildWriteFrame(ushort startAddress, TData[] values)
    {
        return new NativeWriteFrame<TData>(startAddress, values);
    }
    ...
    /// 创建数据映射（批量写入构建器）
    /// </summary>
    public IDataMapping<TProtocol, TData> CreateDataMapping()
    {
